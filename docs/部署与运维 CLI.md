...
Manager 是同一份。左边列的是**部署**（`主机/部署名`）而不是主机 —— 一台机可以跑
好几个 Manager。
...
| 分组 | 能做什么 |
|---|---|
| 侧栏 | 加主机、在已有主机上再加一个 Manager |
| 概览 | 状态、日志、**显示完整 token**、流水 |
| 运维 | 停机备份、升级到指定镜像、回滚上一个 |
| 危险动作 | 停并删容器（保留 `data/`）、连 `data/` 一起删（**要把 `主机/部署名` 原样打一遍**） |
| 这条记录 | 从库里删部署 / 删整台主机（只动本地库，不碰目标机） |
| 部署 | 体检 + 预览（含将写入的文件全文）、部署（进度 SSE 流式打出来） |
| Profile | 列表、启动、停止、全部停止 |
| 插件 | 推送、下发、验证、查漂移 |

只有 `sleight tunnel` 没有界面对应 —— 那个要在你自己的终端里持续跑着。

Manager 那边**没有用户名密码**，`AUTH_TOKEN` 就是唯一凭据：`/api/status` 免鉴权，
其余接口没有 token 一律 401。点"显示 token"拿到之后填进 Manager 自己的 Web UI 即可。
